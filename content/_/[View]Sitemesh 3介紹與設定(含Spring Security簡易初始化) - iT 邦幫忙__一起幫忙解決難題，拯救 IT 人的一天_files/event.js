$(function () {
                    ...
                    if (isConfirm) {
                        $.post('/api/report', {
                            ...
                            if (result.status == "success") {
                                ...
                                sweetAlert('檢舉成功', '', 'success');
                            } else {
                                sweetAlert('檢舉失敗', '', 'error');
                            }
                        });
                    } else {
                        sweetAlert.close();
    ...
    $(document).on('click', '.report_talk_btn', function () {
        var $this = $(this);
        var id = $this.data('id');
        var type = $this.data('type');
    ...
    $('.check_forbid').find(':submit').click(function (event) {
        ...
        $.get('/api/forbid', form_data).then(function (result) {
            if (!result.status) {
                sweetAlert({
                    title: 'Tip',
                    text: result.message,
